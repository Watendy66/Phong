...
def render():
    for i, j in pixels:
        ...
        if t_min < 1e10:
            ...
            # ===== 阴影检测 =====
            shadow_ro = hit_point + N * 1e-4
            shadow_rd = L
            dist_to_light = (light_pos - hit_point).norm()
            
            is_shadowed = False
            
            t_sh_sphere, _ = intersect_sphere(shadow_ro, shadow_rd, sphere_center, sphere_radius)
            if 0.0 < t_sh_sphere < dist_to_light:
                is_shadowed = True
                
            if not is_shadowed:
                t_sh_cone, _ = intersect_cone(shadow_ro, shadow_rd, cone_vertex, cone_base_y, cone_base_radius)
                if 0.0 < t_sh_cone < dist_to_light:
                    is_shadowed = True
