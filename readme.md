...
## 🚀 进阶实现 (Advanced / Bonus Features)

<!-- 如果你完成了选做内容，请在下方补充你的实现思路和对比图 -->

### 1. Blinn-Phong 光照模型升级
*（请在此处描述 Blinn-Phong 的实现原理。例如：引入了半程向量 $\mathbf{H}$，替代了原有的反射向量 $\mathbf{R}$。并简述其在高光区域边缘大入射角时的视觉表现与原 Phong 模型的差异。）*

**对比效果：**
- Phong 模型表现：[图片占位]
- Blinn-Phong 模型表现：[图片占位]

### 2. 硬阴影 (Hard Shadow)
*（请在此处描述硬阴影的实现思路。例如：向光源方向发射一条暗影射线 (Shadow Ray)，如果在到达光源前击中了其他物体，则判定该点处于阴影中，仅计算环境光。）*
